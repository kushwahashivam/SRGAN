...
from models import Generator, Discriminator, VGGNet

root_train = "data/DIV2K_train_HR/"
root_val = "data/DIV2K_valid_HR/"
gen_dir = "model/vgg/generator.pt"
disc_dir = "model/vgg/discriminator.pt"
gen_optim_dir = "model/vgg/gen_optim.pt"
disc_optim_dir = "model/vgg/disc_optim.pt"
gs_dir = "summary/vgg_gs.pkl"
log_dir = "summary/"
...
vgg_optim = torch.optim.Adam(vgg.parameters())
...
if not retrain:
    if os.path.exists("model/mse/generator.pt"):
        state_dict = torch.load("model/mse/generator.pt", map_location=device)
        gen.load_state_dict(state_dict)
        del state_dict
    else:
        raise FileNotFoundError("Generator model not found.")
...
def train_gen(lr_img, hr_img, labels):
    ...
    mse_loss = mse_criterion(sr_features, hr_features)
...
for lr_img, hr_img in tqdm(dl_train):
    ...
    disc_optim.step()
    loss = loss.item()
    writer.add_scalar("vgg_training/loss/disc_loss_real", loss, gs)
    # Train discriminator on fake data
    labels = torch.zeros(batch_size, 1).to(device)
    loss = train_disc(lr_img, labels)
    writer.add_scalar("vgg_training/loss/disc_loss_fake", loss, gs)
    # Train generator
    labels = torch.ones(batch_size, 1).to(device)
    loss = train_gen(lr_img, hr_img, labels)
    writer.add_scalar("vgg_training/loss/gen_loss", loss, gs)
    if (gs)%100 == 0:
        ...
        hr_img = hr_img * .5 + .5
        sr_img = sr_img * .5 + .5
        writer.add_image("vgg_training/image/original_images", tv.utils.make_grid(hr_img, nrow=2), 0)
        writer.add_image("vgg_training/image/superresolved_images", tv.utils.make_grid(sr_img, nrow=2), 0)
        gen.train()
        writer.flush()
    gs += 1
writer.flush()
